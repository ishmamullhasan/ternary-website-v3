...
const run = async () => {
  ...
  for (const d of ind.docs as { id: string; title?: string; slug?: string; _status?: string }[]) {
    ...
    console.log(
      `  ${String(d.id).slice(-6)} | ${(d.title ?? '∅').padEnd(34)} | slug=${d.slug ?? '∅'} | ${d._status ?? '?'}${mismatch}`,
    )
  }
  ...
  for (const c of [
    ...
    try {
      ...
      const copies = (all.docs as { slug?: string; title?: string }[]).filter(
        (d) => /---copy|—copy|copy$/i.test(d.slug ?? '') || /copy/i.test(d.title ?? ''),
      )
      console.log(
        `  ${c.padEnd(13)} total=${all.totalDocs}  ---copy-ish=${copies.length}  ${copies
          .slice(0, 6)
          .map((d) => d.slug)
          .join(', ')}`,
      )
