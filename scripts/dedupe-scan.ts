...
const INDUSTRY_DUPE_SLUGS = new Set([
  'oil--gas',
  'technology',
  'consumer--retail',
  'hospitality--travel',
...
const run = async () => {
  ...
  const collect = async (coll: string, pick: (d: { slug?: string }) => boolean) => {
    ...
    for (const d of res.docs as { id: string; slug?: string; title?: string }[])
      if (pick(d)) candidates.push({ id: String(d.id), collection: coll, slug: d.slug, title: d.title })
  }
  await collect('industry', (d) => INDUSTRY_DUPE_SLUGS.has(d.slug ?? ''))
  for (const c of ['solution', 'insight', 'pressRelease', 'story', 'capability', 'model'])
    await collect(c, (d) => COPY_RE.test(d.slug ?? ''))

  // Build the reference haystack: globals + all pages (stringified, depth 0).
  const haystacks: { where: string; json: string }[] = []
  for (const g of ['footer', 'header'])
    haystacks.push({
      where: `global:${g}`,
      json: JSON.stringify(await payload.findGlobal({ slug: g as never, depth: 0 })),
    })
  const pages = await payload.find({ collection: 'pages' as never, depth: 0, limit: 200, pagination: false })
  for (const p of pages.docs as { id: string; slug?: string }[])
    haystacks.push({ where: `page:${p.slug ?? p.id}`, json: JSON.stringify(p) })
  ...
  if (DELETE) {
    ...
    for (const c of toDelete) {
      try {
        ...
        if (BENIGN(e)) {
          n++
          console.log(`  ✓ deleted [${c.collection}] ${c.slug} (revalidateTag skipped)`)
