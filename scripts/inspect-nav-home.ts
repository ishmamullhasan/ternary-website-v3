...
const run = async () => {
  ...
  ).docs?.[0] as { layout?: { blockType?: string }[] } | undefined
  console.log('\n=== home layout order (index : blockType) ===')
  ;(page?.layout ?? []).forEach((b, i) => console.log(`  [${i}] ${b.blockType}`))

  const h = (await payload.findGlobal({ slug: 'header' as never, locale: 'en', depth: 0 })) as {
    menu?: { label?: string; link?: string; subItems?: unknown[] }[]
