...
const DEFAULT_COMPARE_ROWS: { label: string; cells: (ReactNode | string)[]; tabular?: boolean }[] = [
  {
    label: 'Best when',
    cells: [
...
// ---------------------------------------------------------------------------------------------
import type { SolutionsHubBlock } from '@/payload-types'

/** Bold the canonical model names inside a plain-text compare cell. */
function boldModels(cell: string): ReactNode {
  const parts = cell.split(/(Frame|Flow|Orchestra)/)
  if (parts.length === 1) return cell
  return parts.map((p, i) =>
    /^(Frame|Flow|Orchestra)$/.test(p) ? (
      <b key={i} className="font-medium text-cream">
        {p}
      </b>
    ) : (
      p
...
export function SolutionsHubComponent(cms: Partial<SolutionsHubBlock> = {}): JSX.Element {
  // CMS-first with the original hardcoded content as the fallback, so the page can never render
  // empty while the doc is being authored.
  const SOLUTIONS = cms.solutions?.length
    ? cms.solutions.map((s, i) => ({
        id: `s${i + 1}`,
        art: i,
        name: s.name ?? '',
        tagline: s.tagline ?? '',
        ...
        what: s.what ?? '',
        get: s.get ?? '',
        proof: s.proofClient ? { client: s.proofClient, body: s.proofBody ?? '' } : null,
      }))
    : DEFAULT_SOLUTIONS
  const MODELS = cms.models?.length
    ? cms.models.map((m) => ({ name: m.name ?? '', tag: m.tag ?? '', body: m.body ?? '', ideal: m.ideal ?? '' }))
    : DEFAULT_MODELS
  const COMPARE_COLS = cms.solutions?.length ? cms.solutions.map((s) => s.name ?? '') : DEFAULT_COMPARE_COLS
  const COMPARE_ROWS = cms.compareRows?.length
    ? cms.compareRows.map((r) => ({
        label: r.label ?? '',
        tabular: r.tabular ?? false,
        cells: (r.cells ?? []).map((c) => boldModels(c.cell ?? '')),
      }))
    : DEFAULT_COMPARE_ROWS
  const heroHeading = cms.heroHeading || 'Built to outlast.'
  const heroSub =
    cms.heroSub ||
  ...
  const ctaHeading = cms.ctaHeading || 'Still not sure which one you need?'
  const ctaBody =
    cms.ctaBody ||
