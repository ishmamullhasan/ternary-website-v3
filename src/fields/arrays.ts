...
export const cardsArray = ({
  ...
  if (link) fields.push({ name: 'link', label: 'Link', type: 'text', admin: { description: 'Optional URL the card links to.' } })
  if (span)
    fields.push({
      name: 'span',
      label: 'Tile size',
...
export const statsArray = ({ name = 'stats', label = 'Stats' } = {}): ArrayField => ({
  ...
  fields: [
    { name: 'value', label: 'Value', type: 'text', required: true, admin: { width: '33%' } },
    { name: 'label', label: 'Label', type: 'text', required: true, admin: { width: '33%' } },
    { name: 'detail', label: 'Detail', type: 'text', admin: { width: '34%' } },
  ],
})
...
export const tagsArray = ({ name = 'tags', label = 'Tags' } = {}): ArrayField => ({
  ...
  type: 'array',
  admin: { ...rowLabelAdmin },
  fields: [{ name: 'name', label: 'Tag', type: 'text', required: true }],
})
