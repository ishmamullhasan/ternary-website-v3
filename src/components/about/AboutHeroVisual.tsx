...
import './aboutHeroVisual.css'

gsap.registerPlugin(ScrollTrigger)

/**
 * The About hero's structure, animated as a 2.5D object.
 *
 ...
 */
export default function AboutHeroVisual(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      mm.add({ wide: '(min-width: 1024px)', narrow: '(max-width: 1023px)' }, (context) => {
        ...
        const float = root.querySelector<HTMLElement>('.ahv-float')
        const breathe = root.querySelector<HTMLElement>('.ahv-breathe')
        const frame = root.querySelector<HTMLElement>('.ahv-frame')
        const sweep = root.querySelector<HTMLElement>('.ahv-sweep')
        const dots = [...root.querySelectorAll<SVGCircleElement>('.ahv-dot')]
        ...
        if (wide && window.matchMedia('(hover: hover)').matches) {
          const dotLayer = root.querySelector<SVGGElement>('.ahv-dot-layer')
  ...
  return (
    <div ref={ref} className="ahv-visual">
      <div className="ahv-stage">
        <div className="ahv-float">
          <div className="ahv-breathe">
            <div className="ahv-frame">
              <Image
                ...
                className="ahv-img"
              />
              <span aria-hidden className="ahv-sweep" />
            </div>
          </div>
        </div>
      </div>

      <svg aria-hidden className="ahv-dots" viewBox="0 0 100 100" preserveAspectRatio="none" focusable="false">
        <g className="ahv-dot-layer">
          {dots.map((d, i) => (
            <circle key={i} className="ahv-dot" cx={d.x} cy={d.y} r="0.32" fill="#f2f0ea" opacity="0.55" />
          ))}
        </g>
      </svg>
    </div>
