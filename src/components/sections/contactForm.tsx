  ...
  return (
    <aside className="flex w-full shrink-0 flex-col justify-between gap-10 lg:w-[278px]">
      <div className="space-y-4">
        <p className="text-[12px] uppercase tracking-[0.14em] text-subtle">Intake flow</p>
          ...
          {STEPS.map((step, i) => {
            ...
            return (
              <li
                ...
                className={[
                  'flex items-center justify-between gap-3 rounded-md border px-4 py-3 transition-colors duration-300',
                  current
                    ? 'border-cream/70 bg-main text-cream'
                    : done
                      ? 'border-line bg-main text-body'
              ...
              >
                <span className="flex items-center gap-3">
                  ...
                  <span className="text-[15px]">{step.title}</span>
                </span>
                <span className="text-[12px] tabular-nums tracking-[0.08em] text-subtle">{step.meta}</span>
              </li>
            )
          })}
        </ol>
      ...
      <div className="space-y-2 rounded-md border border-line bg-main p-5">
        <p className="text-[12px] uppercase tracking-[0.14em] text-subtle">Field count</p>
        <p className="text-[15px] text-cream">9 questions · ~3 minutes</p>
        <p className="text-[13px] leading-relaxed text-subtle">Budget is optional. Anything else can be left blank.</p>
      </div>
    </aside>
  )
}
...
function FieldGroup({
  ...
  return (
    <div className="space-y-3">
      <p id={htmlId} className="text-[13px] text-subtle">
        {label}
      </p>
      {children}
    </div>
...
export default function ContactForm({ formId, submitLabel, confirmationFallback }: ContactFormProps): JSX.Element {
  ...
  if (status === 'success') {
    return (
      <div className="grid place-items-center rounded-md border border-line bg-ink p-10 text-center lg:min-h-[420px]">
        <div className="max-w-md space-y-4">
          <span className="mx-auto flex size-12 items-center justify-center rounded-full border border-emerald-400/40 text-emerald-400">
  ...
  return (
    <div className="grid gap-8 rounded-md border border-line bg-main p-6 md:p-9 lg:grid-cols-[278px_1fr] lg:gap-12">
      <StepperRail activeStep={activeStep} />

      <form
        onSubmit={handleSubmit}
        className="flex flex-col gap-8 rounded-md border border-line bg-ink p-6 md:p-8"
        noValidate
      >
        {/* Step header + progress bars */}
        <div className="flex items-center justify-between gap-4">
          <p className="text-[13px] text-subtle">
            Step {activeStep + 1}/{totalSteps} · <span className="text-body">Context &amp; timing</span>
          </p>
        ...
        <FieldGroup label="Timeline" htmlId={timelineLabelId}>
          <div role="radiogroup" aria-labelledby={timelineLabelId} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {TIMELINE.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={[
                    'group relative flex flex-col gap-1 rounded-md border px-4 py-3 text-left transition-colors duration-300',
                    focusRing,
                    selected
                      ? 'border-cream/80 bg-main'
                      : 'border-line bg-main/40 hover:border-line-strong hover:bg-main',
                  ].join(' ')}
                >
                  <span className="flex items-center justify-between">
                    <span className={`text-[15px] ${selected ? 'text-cream' : 'text-body'}`}>{opt.title}</span>
                    {selected && <Check size={15} strokeWidth={2.5} className="text-emerald-400" aria-hidden />}
                  </span>
                  <span className="text-[13px] text-subtle">{opt.subtitle}</span>
                </button>
              )
            })}
          </div>
        ...
        <FieldGroup label="Budget band" htmlId={budgetLabelId}>
          <div role="radiogroup" aria-labelledby={budgetLabelId} className="flex flex-wrap gap-2.5">
            {BUDGET.map((opt) => {
                ...
                <button
                  ...
                  className={[
                    ...
                    selected
                      ...
                      : 'border-line-strong bg-main text-subtle hover:border-subtle hover:text-body',
                  ].join(' ')}
                >
                  {opt}
                </button>
        ...
        <FieldGroup label="What brought you here?" htmlId={sourceLabelId}>
          <div role="group" aria-labelledby={sourceLabelId} className="flex flex-wrap gap-2.5">
            {SOURCES.map((opt) => {
        ...
        <FieldGroup label="Anything else we should know?">
          <textarea
            ...
            className={`w-full resize-y rounded-md border border-line-strong bg-main px-4 py-3 text-[15px] text-cream placeholder:text-subtle transition-colors hover:border-subtle focus:border-cream/70 ${focusRing}`}
          />
        </FieldGroup>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="flex items-center gap-2 text-[13px] text-subtle">
            <ShieldCheck size={15} aria-hidden className="text-subtle" />
            By submitting you agree to our{' '}
            <Link
              href="/legals/privacy-and-policy"
              className={`text-body underline-offset-4 hover:text-cream hover:underline ${focusRing} rounded-sm`}
            >
              privacy practices
            </Link>
            .
          ...
          <div className="flex items-center gap-3">
            <button
              type="button"
              className={`rounded-full border border-line-strong px-5 py-2.5 text-[14px] font-medium text-body transition-colors hover:border-subtle hover:text-cream ${focusRing}`}
            >
              Back
            </button>
            <button
              ...
              className={`group inline-flex items-center gap-2 rounded-full bg-cream px-6 py-2.5 text-[14px] font-medium text-ink transition-colors hover:bg-cream-hover disabled:cursor-not-allowed disabled:opacity-60 ${focusRing}`}
            >
